...
class Client:
	...
	def createWidgets(self):
		...
		# Create Setup button
		# Create Play button		
		self.start = Button(self.master, width=24, padx=3, pady=3)
		self.start["text"] = "Play"
		self.start["command"] = self.playMovie
		self.start.grid(row=1, column=1, padx=2, pady=2)
		
		# Create Pause button			
		self.pause = Button(self.master, width=24, padx=3, pady=3)
		self.pause["text"] = "Pause"
		self.pause["command"] = self.pauseMovie
		self.pause.grid(row=1, column=2, padx=2, pady=2)
		
		# Create Teardown button
		self.teardown = Button(self.master, width=24, padx=3, pady=3)
	...
	def recvRtspReply(self):
		...
		while True:
			...
			# '''
			if not self.outOfRuntime and (
					self.requestSent == self.PAUSE or self.state == self.INIT and self.requestSent == self.TEARDOWN):
				self.playTime += datetime.now() - self.startTime
			...
			if self.requestSent == self.TEARDOWN:
				...
				if self.totalPacketsReceived > 0:
					...
					# Packet loss rate = (number of lost packets)/(total number of packets)
					...
					# Number of Packets Received
					print("Number of Packets Received: " + str(self.totalPacketsReceived))
					# Packet loss rate = no packets sent/lost
					print("Packet loss rate: " + str(self.packetLossRate))
					# Number of Bytes Received
					print("Number of Bytes Received: " + str(self.numberOfBytesReceived))
					# playTime
	...
	def parseRtspReply(self, data):
		...
		print ("\nServer:\n" + decodedData) #print the server's response.
